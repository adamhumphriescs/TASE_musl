...
static TASELDOUBLE wcstox(const wchar_t *s, wchar_t **p, int prec)
{
	wchar_t *t = (wchar_t *)s;
	...
	f.cookie = (void *)t;
	shlim(&f, 0);
	TASELDOUBLE y = __floatscan(&f, prec, 1);
	if (p) {
		size_t cnt = shcnt(&f);
...
TASELDOUBLE wcstold(const wchar_t *restrict s, wchar_t **restrict p)
{
	return wcstox(s, p, 2);
}
